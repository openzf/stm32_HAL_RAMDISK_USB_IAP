 ...
 * GLOBAL VARIABLES
 */
static Ymodem_TypeDef g_ymodem; // 全局变量
...
uint8_t ymodem_rx_prepare( char *buf, size_t sz ) //解析出头包中的文件名和大小
{
  ...
  ans = g_ymodem.ymodem_rx_header( fil_nm, fil_sz );
  return ans;
}
/*********************************************************************
...
void ymodem_rx_put( char *buf, size_t rx_sz )
{
  ...
  case YMODEM_RX_IDLE:
    ...
    case EOT:
      ...
      goto exit;                      //这是在本循环必须完成的操作，所以需要用到 goto 语句
    default:
//     g_ymodem.ymodem_write_char( NAK );      //不正常的状态，调试用
    ...
    case YMODEM_RX_ACK:                                         //1级——文件接收状态中
      switch( ymodem_rx_pac_check( buf, rx_sz ) )   //检查当前包是否合法,并返回包的类型
      {
        ...
        case STX:
          g_ymodem.ymodem_write_char( ACK );
          pac_size = (uint8_t)(buf[0])==SOH? PACKET_SIZE:PACKET_1K_SIZE;
          g_ymodem.ymodem_rx_pac_get( buf+PACKET_HEADER, seek, pac_size );  //将接收的包保存
          seek += pac_size;
          g_ymodem.ymodem_write_char( 'C' );
          break;
          //指令包
        case EOT:
          g_ymodem.ymodem_write_char( NAK );
...
//          goto err;           //这儿暂时认为，包有误，就重发
    ...
    case YMODEM_RX_EOT:         //在这里保存文件
    {
      switch( ymodem_rx_pac_check( buf, rx_sz ) )   //检查当前包是否合法,并返回包的类型
      {
        ...
        case EOT:
          g_ymodem.ymodem_write_char( ACK );
          g_ymodem.ymodem_rx_finish( YMODEM_OK );        //确认发送完毕，保存文件
    ...
    case CNC:
      {
        if( NULL == ym_tx_pbuf )
        {
          ym_tx_pbuf = YMODEM_MALLOC( PACKET_OVERHEAD + PACKET_1K_SIZE );
          if( NULL == ym_tx_pbuf )      //申请失败，则返回
            break;
        }
        if( YMODEM_OK == g_ymodem.ymodem_tx_header( &fil_nm, &fil_sz ) )   //得到 文件名和大小
        {//封装一个包头，然后发送出去
          ym_tx_fil_sz = fil_sz;
          ymodem_tx_make_pac_header( ym_tx_pbuf, fil_nm, fil_sz );
          g_ymodem.ymodem_write_str( ym_tx_pbuf, PACKET_OVERHEAD+PACKET_SIZE );
...
dt_tx: case YMODEM_TX_DATA:                             //1级——文件发送状态中
      ...
      {
        case CNC:
          if( YMODEM_OK == g_ymodem.ymodem_tx_pac_get( ym_tx_pbuf+PACKET_HEADER, seek, PACKET_1K_SIZE ) )  //读取下一组数据
          {
            if( YMODEM_OK == ymodem_tx_make_pac_data( ym_tx_pbuf, PACKET_1K_SIZE ) )
            {
              g_ymodem.ymodem_write_str( ym_tx_pbuf, PACKET_OVERHEAD+PACKET_1K_SIZE );
    ...
    case YMODEM_TX_DATA_ACK:
      {
        ...
        case ACK:
          ...
          else  //数据发送完
          {
            ...
            g_ymodem.ymodem_write_char( EOT );
          }
        break;
    ...
    case YMODEM_TX_EOT:
    {
      switch( ymodem_rx_pac_check( buf, rx_sz ) )   //检查当前包是否合法,并返回包的类型
      {
        ...
        case ACK:
          g_ymodem.ymodem_write_char( ACK );
          g_ymodem.ymodem_tx_finish( YMODEM_OK );
...
err_tx:  case YMODEM_TX_ERR:         //在这里放弃保存文件,终止传输
  ...
  case YMODEM_TX_EXIT:        //到这里，就收拾好，然后退出
      ym_rx_status = YMODEM_RX_IDLE;
      //*这里还需要进行某些操作，使在退出后，不会再重新进入ymodem_rx_put()函数
      YMODEM_FREE( ym_tx_pbuf );
      ym_tx_pbuf = NULL;
      g_ymodem.ymodem_tx_exit();
      return;
    default:
      break;
  }
